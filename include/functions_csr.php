...
function createCSR_form()
{
  ...
  <form action="index.php" method="post">
    ...
    <table style="width: 90%;">
      ...
      <tr>
        <th>Key Size</th>
        <td><input type="radio" name="cert_dn[keySize]" value="1024" /> 1024bits <input type="radio"
            name="cert_dn[keySize]" value="2048bits" /> 2048bits<input type="radio" name="cert_dn[keySize]"
            value="4096bits" checked /> 4096bits</td>
      </tr>
      <tr>
        <th>Device Type</th>
      ...
      <tr>
        <td>
        <td><input type="submit" value="Create CSR" />
    </table>
  </form>
  </p>
  <?PHP
...
//,'private_key_bits'=>$config['private_key_bits'],'x509_extensions'=>$config['x509_extensions']);
  ...
  <table style="width: 90%;">
    <tr>
      <th width=100>Common Name (eg www.golf.local)</th>
      <td>
        <?PHP print $my_details['CN']; ?>
      </td>
    </tr>
    <tr>
      <th>Contact Email Address</th>
      <td>
        <?PHP print $my_details['emailAddress']; ?>
      </td>
    </tr>
    <tr>
      <th>Organizational Unit Name</th>
      <td>
        <?PHP print $my_details['OU']; ?>
      </td>
    </tr>
    <tr>
      <th>Organization Name</th>
      <td>
        <?PHP print $my_details['O']; ?>
      </td>
    </tr>
    <tr>
      <th>City</th>
      <td>
        <?PHP print $my_details['L']; ?>
      </td>
    </tr>
    <tr>
      <th>State</th>
      <td>
        <?PHP print $my_details['ST']; ?>
      </td>
    </tr>
    <tr>
      <th>Country</th>
      <td>
        <?PHP print $my_details['C']; ?>
      </td>
    </tr>
    <tr>
      <th>Key Size</th>
      <td>
        <?PHP print $my_public_key_details['bits']; ?>
      </td>
    </tr>
  </table>
  <?PHP
  ...
  <form action="index.php" method="post">
    ...
    <table style="width: 90%;">
      ...
      <tr>
        ...
        <td><input type="radio" name="rename_ext" value="FALSE" checked />Do not Rename<br><input type="radio"
            name="rename_ext" value="cer" /> Rename to cer<br><input type="radio" name="rename_ext" value="csr" /> Rename
          to csr<br></td>
      </tr>
      <?PHP
      /*
...
function sign_csr_form($my_values = array('csr_name' => '::zz::'))
{
  ...
  <p>
    ...
    <form action="index.php" method="post">
      ...
      <table style="width: 90%;">
        <tr>
          <td width=100>CA Passphrase:
          <td><input type="password" name="pass" />
        <tr>
          <td width=100>Number of days Certificate is to be valid for:
              ...
              <option value="">--- Select a CSR
                ...
                while (($file = readdir($dh)) !== false) {
                  if (($file !== ".htaccess") && is_file($config['req_path'] . $file)) {
                    ...
                    $ext = substr($file, strrpos($file, '.'));
                    if (!is_file($config['cert_path'] . $file) or ($my_values['csr_name'] == "$name$ext")) {
                      if ($my_values['csr_name'] == "$name$ext")
        ...
        <tr>
          <td>
          <td><input type="submit" value="Sign CSR">
      </table>
    </form>
    <?php
    } else
