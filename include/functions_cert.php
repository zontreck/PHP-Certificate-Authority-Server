    ...
    <table style="width: 400px;">
      ...
      <tr>
        ...
        <td><input type="radio" name="rename_ext" value="FALSE" checked />Do not Rename<br><input type="radio"
            name="rename_ext" value="cer" /> Rename to cer<br><input type="radio" name="rename_ext" value="pfx" /> Rename
          to pfx<br></td>
      </tr>
      <?PHP
      /*
      ...
      */
      ...
      <tr>
        <td width=100>Name:
        <td><select name="cert_name" rows="6">
      ...
      <tr>
        <td>
        <td><input type="submit" value="Download Certificate">
    </table>
  </form>
  </p>
  <?PHP
...
function convert_cert_pkcs12_form($my_values = array('cert_name' => '::zz::'))
{
  ...
  <p>
    ...
    <form action="index.php?menuoption=download_cert" method="post">
      ...
      <table style="width: 400px;">
        <tr>
          <td width=100>Private Key Passphrase:
          <td><input type="password" name="pkey_pass" />
        <tr>
          <td width=100>PKCS#12 Passphrase:
          <td><input type="password" name="pkcs12_pass" />
        <tr>
          <td width=100>Name:
          <td><select name="cert_name" rows="6">
              <option value="">--- Select a certificate
                ...
                while (($file = readdir($dh)) !== false) {
                  if (($file !== ".htaccess") && is_file($config['cert_path'] . $file) && (substr($file, strrpos($file, '.')) != '.p12') && !is_file($config['cert_path'] . substr($file, 0, strrpos($file, '.')) . '.p12')) {
                    if (is_file($config['key_path'] . $file)) {
                      $name = base64_decode(substr($file, 0, strrpos($file, '.')));
                      $ext = substr($file, strrpos($file, '.'));
                      if ($my_values['cert_name'] == "$name$ext")
        ...
        <tr>
          <td>
          <td><input type="submit" value="Convert Certificate">
      </table>
    </form>
    <?php
    } else
...
function revoke_cert_form($my_values = array('cert_serial' => -99))
{
  ...
  <form action="index.php" method="post">
    ...
    <table style="width: 400px;">
            ...
            <option value="">--- Select a certificate
              ...
              while (!feof($my_index_handle)) {
                ...
                if (preg_match($pattern, $this_line, $matches)) {
                  if ($matches[1] == 'V') {
                    if ($my_values['cert_serial'] == $matches[4])
                      $this_selected = " selected=\"selected\"";
                    else
                      $this_selected = "";
      ...
      <tr>
        <td>
        <td><input type="submit" value="Revoke Certificate">
    </table>
  </form>
  </p>
  <?PHP
  ...
  <p>
    ...
    <form action="index.php" method="post">
      ...
      <table style="width: 400px;">
        ...
        <tr>
          <td>
          <td><input type="submit" value="View Certificate">
      </table>
    </form>
    <?php
    } else
